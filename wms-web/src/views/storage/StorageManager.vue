...
const getUserPageInfo =async ()=>{
  let res:storageResponse = await storageStore.storageInfo({
...
const remove = async (id:number)=>{
  try {
    await storageStore.storageDelete(id)
    ElMessage({
      type:'success',
      message:'删除成功'
    })
...
const save = async ()=>{
  ...
  try{
    await storageStore.storageModOrAdd(form)
    ElMessage({
      type:'success',
...
const validateName = (rule:any,value: any, callback: any)=>{
  if (value.length >= 3 && value.length <= 8){
    ...
    callback(new Error("仓库名长度在3~8个字符之间"))
  }
}
...
<template>
  <div>
    <el-input v-model="name" placeholder="请输入仓库名" style="width:200px;margin-right: 10px;" :prefix-icon="Search" @keyup.enter="getUserPageInfo"></el-input>
